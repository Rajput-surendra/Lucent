...
class _BookingDetailsScreenState extends State<BookingDetailsScreen> {
  ...
  getplanPurchaseSuccessApi() async {
    ...
    var request = http.MultipartRequest(
        'POST', Uri.parse('${ApiService.getPlanPurchasApi}'));
    ...
    if (response.statusCode == 200) {
      ...
      Navigator.push(context,
          MaterialPageRoute(builder: (context) => const PlanSuccessScreen()));
  ...
  genratePaytmApi() async {
    ...
    var request = http.MultipartRequest(
        'POST', Uri.parse('${ApiService.getGeneratePaytmApi}'));
    request.fields.addAll({
      'amount': changePrice ? disCountAmount : widget.amount,
      'order_id': '2000',
      'user_id': userId.toString()
  ...
  checkPromoCode() async {
    ...
    var request = http.MultipartRequest(
        'POST', Uri.parse('${ApiService.checkPromoCodeApi}'));
    request.fields.addAll({
      'user_id': userId.toString(),
  ...
  getPromoCodeApi() async {
    ...
    var request =
        http.Request('GET', Uri.parse('${ApiService.getPromoCodeApi}'));
    request.headers.addAll(headers);
    http.StreamedResponse response = await request.send();
    if (response.statusCode == 200) {
      var result = await response.stream.bytesToString();
          ...
          child: SingleChildScrollView(
            child: Column(
              children: [
                ...
                getPromoCodeModel == null || getPromoCodeModel == ""
                    ? const CircularProgressIndicator()
                    : changePrice
                        ? const SizedBox.shrink()
                        : getPromoCodeModel!.data!.length > 0
                            ? applyPromoCode()
                            : const SizedBox.shrink(),
                Container(
                                                  ...
                                                  Row(
                                                    ...
                                                    children: [
                                                      ...
                                                      Column(
                                                        ...
                                                        children: [
                                                          Text(
                                                            "${getPromoCodeModel!.data![index].code}",
                                                            style: const TextStyle(
                                                                color: colors
                                                                    .blackTemp),
                                                          ),
                                                          ...
                                                          Text(
                                                            "₹ ${getPromoCodeModel!.data![index].discount}",
                                                            style: const TextStyle(
                                                                color: colors
                                                                    .blackTemp),
                                                          )
